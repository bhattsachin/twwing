...
public final class AztecWriter implements Writer {
  ...
  @Override
  public BitMatrix encode(String contents, BarcodeFormatNow format, int width, int height) {
    return encode(contents, format, DEFAULT_CHARSET, Encoder.DEFAULT_EC_PERCENT);
  }

  @Override
  public BitMatrix encode(String contents, BarcodeFormatNow format, int width, int height, Map<EncodeHintType,?> hints) {
  ...
  private static BitMatrix encode(String contents, BarcodeFormatNow format, Charset charset, int eccPercent) {
    if (format != BarcodeFormatNow.AZTEC) {
